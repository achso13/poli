                    ...
                    <?php foreach ($result as $row) : ?>
                        <tr>
                            ...
                            <td width="25%">
                                ...
                                <button class="btn btn-info btn-sm btn-edit" uc="<?= $row['id_biro'] ?>" data-toggle="modal" data-target="#form-modals">
                                    <i class="mr-1 fa fa-pen-square"></i> Edit
                                </button>

                                <button class="btn btn-danger btn-sm" data-toggle="modal" data-target="#modals-delete-<?= $row['id_biro'] ?>">
                                    <i class="mr-1 fa fa-trash-alt"></i> Delete
                                </button>

                                <div class="modal fade" id="modals-delete-<?= $row['id_biro'] ?>" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <?= form_open('unitkerja/biro/delete/' . $row['id_biro']) ?>
                                                <?= csrf_field() ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('.btn-add').click(function() {
            $('.load-form').load(base_url + '/unitkerja/biro/add');
        });

        $('.btn-edit').click(function() {
            var id = $(this).attr('uc');

            $('.load-form').load(base_url + '/unitkerja/biro/edit', {
                id: id
            });
        });
