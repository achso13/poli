            ...
            <table class="table table-bordered table-hover" width="100%" cellspacing="0" id="dataTable">
                <thead>
                    <tr class="bg-light">
                        ...
                        <td class="text-primary text-center">Judul</td>
                        <td class="text-primary text-center">Isi</td>
                        <td class="text-primary text-center">Pembuat</td>
                ...
                <tbody>
                    ...
                    <?php foreach ($result as $row) : ?>
                        <tr>
                            ...
                            <td>
                                <?= time_format($row['created_at'], "d M Y, H:i") ?>
                            </td>
                            <td width="25%">

                                <a href="<?= base_url('pengumuman/view/' . $row['id_pengumuman']) ?>" class="btn btn-sm btn-white">
                                    <span class="text-light">
                                        <i class="material-icons">visibility</i>
                                    </span> Lihat
                                </a>
                                <?php if (session()->get('log_role') === "ADMIN") :  ?>
                                    <button class="btn btn-info btn-sm btn-edit" uc="<?= $row['id_pengumuman'] ?>" data-toggle="modal" data-target="#form-modals">
                                        <i class="mr-1 fa fa-pen-square"></i> Edit
                                    </button>

                                    <button class="btn btn-danger btn-sm" data-toggle="modal" data-target="#modals-delete-<?= $row['id_pengumuman'] ?>">
                                        <i class="mr-1 fa fa-trash-alt"></i> Delete
                                    </button>

                                    <div class="modal fade" id="modals-delete-<?= $row['id_pengumuman'] ?>" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    <?= form_open('pengumuman/delete/' . $row['id_pengumuman']) ?>
                                                    <?= csrf_field() ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('.btn-add').click(function() {
            $('.load-form').load(base_url + '/pengumuman/add');
        });

        $('.btn-edit').click(function() {
            var id = $(this).attr('uc');

            $('.load-form').load(base_url + '/pengumuman/edit', {
                id: id
            });
        });
